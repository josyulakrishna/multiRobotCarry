...
class Task:
    ...
    def __init__(self, envname, hidden_size, max_steps, target, pop_size, reps, test_reps, weight_decay, noise_std, sigma):
        self.task = envname
        # self.env_fn = lambda: make_env
        self.repetitions = reps
        self.test_repetitions = test_reps
        # env = self.env_fn(render=False)
        self.action_dim = 4 #env.action_space.shape[0]
        self.state_dim = 28 #env.observation_space.shape[0]
        self.reward_to_fitness = lambda r: r

        self.max_steps = max_steps
        self.pop_size = pop_sizealgorithm

        self.num_workers = mp.cpu_count()

        self.action_clip = lambda a: np.clip(a, -0.5, 0.5)
        self.target = target
...
class Evaluator:
    ...
    def single_run(self):
        ...
        while True:
            ...
            action = np.hstack((action.reshape(2, 2), np.zeros((2, 1)))).ravel()
...
def train(config, logger):
    ...
    workers = [CMAWorker(id, normalizers[id], task_queue, result_queue, stop, config) for id in
    ...
    while True:
        ...
        best = -np.min(cost)
        logger.info(
            'total steps = %8d    test = %+4.0f (%4.0f)    best = %+4.0f (%+4.0f)    elapased time = %4.0f sec' %
